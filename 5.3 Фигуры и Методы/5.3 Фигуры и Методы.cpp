...
class MyClassException : std::exception
{
public:
    std::string name;

    MyClassException(const std::string& name): name(name){}//создаем конструктор
    const char* what() const throw() override // переопределяем метод из std::exception
    {
...
public:
    ...
    Triangle(int a, int b, float A) : Figure("Равнобедренный треугольник", 3) {
        ...
        C = A;
        if (a != c) {
            throw MyClassException("Равнобедренный треугольник не создан. Стороны а и с должны быть равны");
        }
    }
...
int main() {
    ...
    try {
        ...
        std::cout << "Исключение: " << d.what() << std::endl;
    } 

    try {Triangle90 t90(3, 4, 5, 34.0f, 56.0f);
        print_info(t90);
    ...
    try {TriangleAC_ac tAC(3, 4, 30.0f);
        print_info(tAC);
...
try { Quadrangle q1(3, 4, 5, 6, 90, 90, 90, 90);
        print_info(q1);
...
try {Romb r(7, 40.0f);
 ...
 try { Parallellogram p(3, 8, 54.0f);
        print_info(p);
